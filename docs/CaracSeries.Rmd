...
<li> El cambio porcentual de una serie de tiempo $x_t$ entre los periodos $t-1$ y $t$ es aproximadamente $100*\Delta \ln (x_t)$. Esta aproximación funciona mejor cuando el cambio porcentual es pequeño.<a href="https://sbohorquez.github.io/SeriesdeTiempo/CaracSeries.html#/Cambio-porcentual">Demostración</a></li>
...
<li> Esto significa que el cambio proporcional entre los periodos $t$ y $t-1$ es aproximadamente, 
    \begin{equation}
       \Delta  \ln (x_t) = \ln (x_t) - \ln (x_{t-1}) =  \ln (x_{t-1} + \Delta x_t) - \ln (x_{t-1}) \cong \frac{\Delta x_t}{x_{t-1}}</li>
</ul>    




\begin{frame}{Proceso de innovación}

Finalmente, definimos el proceso de innovación . Sea $\{\varepsilon, -\infty < t < \infty \}$ un proceso estocástico (o secuencia aleatoria) y sea $\mathcal{E}_{t-1}$ el $\sigma$-algebra de eventos predecibles cuando el pasado de $\varepsilon_t$ es conocido. Y asumimos
...
Este es un proceso estacionario en el sentido débil, y no auto-correlacionado. Este tipo de proceso se conoce también como {\it ruido blanco}
...
\begin{itemize}
    \item Una de las principales características de las series de tiempo es la presencia de Auto-correlación o correlaci£n serial.
    \item Ustedes posiblemente están familiarizados con el concepto de correlación entre dos variables, la auto-correlación implica correlación entre la serie y su pasado.
\end{itemize}
\end{frame}

\begin{frame}{Correlación}\label{Autoco}


\begin{block}{\emph{Autocovarianza:}}
    ...
    \begin{matrix}
        cov[x_{t_1},x_{t_2}]  & = & E[(x_{t_1}-\mu_{t_1})(x_{t_2}-\mu_{t_2})] \\
                ...
                & & \hyperlink{Prueba:ac}{\beamergotobutton{Demostración}}
    \end{matrix}
    \end{equation}
...
\visible<2-> {La ventaja de usar la auto-correlación en vez de la auto-covarianza es que la auto-correlación siempre va a estar entre -1 y 1, dado que la covarianza siempre es igual o menor que la varianza.} 

\end{frame}   

\begin{frame}{Auto-correlación} 

\begin{itemize}
    \item Las auto-covarianzas y auto-correlaciones expuestas anteriormente son los valores poblacionales, estas pueden ser estimadas en una muestra de tamaño $T$ de la siguiente manera:
    \begin{align}
...
\begin{itemize}
    \item donde $\Bar{x}_{j+1:T}$ es el promedio muestral de $x_t$ usando las observaciones $t=j+1,j+2,\dots,T$
    \item $\widehat{var(x_t)}$ es la varianza muestral de $x_t$.
    \item<2-> Noten que la ecuación de la auto-covarianza se divide por $T$ y no $T-j$ como es usual cuando perdemos grados de libertad, la razón para esto es para poder hacer comparaciones entre diferentes ordenes de las auto-covarianzas.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item La formula en la ecuación de las auto-correlaciones usa el supuesto que la varianza de $x_t$ y $x_{t-j}$ es la misma.
    \item Este supuesto proviene de un supuesto más general llamado estacionareidad que es de fundamental importancia para el análisis de series de tiempo y que discutiremos en detalle más adelante.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item <2-> Estacionareidad: Existen dos tipos de estacionariedad, estacionariedad débil implica que la media y la varianza no dependen del tiempo, mientras que estacionariedad estricta implica que el PGD no depende del tiempo.
\item <3-> Ergodicidad: Una secuencia es ergódica si el promedio de la muestra converge a la esperanza del conjunto. 
\end{itemize}
...
\begin{block}{\emph{Estacionariedad débil:}}
Una secuencia aleatoria $\{x_t\}$ es estacionaria débil (o estacionaria en covarianza) si la media, varianza y la secuencia de autocovarianzas\footnote{La autocovarianza de orden $j$  se define como $\gamma_j = cov(x_{t},x_{t-j})$} de orden $j$, para $j>0$ son independientes de $t$ 
\end{block}
...
\begin{block}{\emph{Estacionariedad Estricta:}}
...
\visible<2->{Estacionariedad estricta implica estacionariedad débil, pero lo contrario no siempre aplica.}

\visible<3->{En el caso especial de la distribución normal, estacionariedad débil si implica estacionariedad estricta.} 
    
\end{frame}

\begin{frame}{Ergodicidad}\label{ergodicidad}

\begin{theorem}
    Si $\{x_t\}$ es una secuencia estacionaria y ergódica, y $E(x_1)$ existe, entonces $\bar{x_n} \rightarrow E(x_1)$ con probabilidad 1.     
\end{theorem}

Donde $\bar{x_n} = \frac{1}{n} \sum_{t=0}^n x_t $  \hyperlink{estacionaria}{\beamergotobutton{Ejemplo}} 

\end{frame}



\begin{frame}{Descomposición de Wold}
...
\visible<2->{En general, si $\upsilon_t = - \sum_{j=0}^{\infty} \frac{\alpha_j}{\alpha_o}\upsilon_{t-j}$ cumple con la condición. Este es un proceso deterministico, ya que puede ser predicho perfectamente un periodo antes. } 
...
\begin{frame}{Procesos no estacionarios}
\begin{itemize}
    \item En economía existen diferentes series que no cumplen con el supuesto de estacionariedad, e.g. el producto interno bruto. Por lo tanto es importante incluir (algunos) procesos de este tipo en nuestros análisis.
\end{itemize}
...
\visible<2->{Si estimamos la media de este proceso vemos que depende del tiempo,
...
\visible<2->{& = & E[x_{t_1}x_{t_2}] - E[x_{t_1}\mu_{t_2}] - E[\mu_{t_1}x_{t_2}] + E[\mu_{t_1}\mu_{t_2}] }\\
\visible<3->{& = & E[x_{t_1}x_{t_2}] - E[x_{t_1}]\mu_{t_2} - \mu_{t_1}E[x_{t_2}] +  \mu_{t_1}\mu_{t_2} }\\
\visible<4->{ & = & E[x_{t_1}x_{t_2}] - \mu_{t_1}\mu_{t_2} - \mu_{t_1}\mu_{t_2} + \mu_{t_1}\mu_{t_2} }\\
\visible<5->{& = & E[x_{t_1}x_{t_2}] - \mu_{t_1}\mu_{t_2}}
\end{matrix}
\end{equation*}
    
\hyperlink{Autoco}{\beamergotobutton{Autocovarianza}}
\end{frame}


\begin{frame}{Ejemplo Serie estacionaria}\label{estacionaria}

   \begin{figure}
       \centering
      \includegraphics[width=2.5in,height=2in]{estacionaria.pdf}
   \end{figure}
    \hyperlink{ergodicidad}{\beamergotobutton{Volver}}
\end{frame}

\begin{frame}{Ejemplo Serie no estacionaria}

   \begin{figure}
       \centering
      \includegraphics[width=2.5in,height=2in]{noestacionaria.pdf}
   \end{figure}
    \hyperlink{ergodicidad}{\beamergotobutton{Volver}}
\end{frame}
