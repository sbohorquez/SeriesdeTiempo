...
Supongamos el siguiente modelo de regresión lineal,
    \begin{equation}
        x_t = \mathbf{Z}_t \boldsymbol{\beta} + u_t, \quad u_t = \rho u_{t-1} + \varepsilon_t, \quad \varepsilon_t \sim IID(0,\sigma^2_{\varepsilon})
    \end{equation}
...
En esta clase hemos hablado de modelos ARMAX, ignorando la posibilidad de un modelo ARIMAX.  La razón para esto es que para estimar este tipo de modelos debemos de hablar de modelos de corrección del error, lo cual se sale de nuestro alcance.

##

A grandes rasgos, lo que necesitamos para este tipo de modelos es que, $x_t - Z_t\beta$ sea estacionario.
 
Lo que haremos para garantizar esta condición  es transformar $x_t$ y $z_t$ en variables estacionarias antes de estimar el modelo ARMAX. 
...
La idea es que la causa no puede venir después del efecto, así si la variable $z_t$ afecta la variable $x_t$, la primera debería ayudar a mejorar la predicción de la segunda.

#
Así entonces lo que testeamos es,
 \begin{equation}\label{eq:gc}
     E(x_{t+h} | \Omega_t) \neq E(x_{t+h} | \Omega_t \backslash \{ z_j | j \leq t \} )  
 \end{equation}
donde $\Omega_t$ es el conjunto de información relevante.Y decimos que existe causalidad instantánea (en el sentido de Granger) si,
\begin{equation}
     E(x_{t+1} | \Omega_t) \neq E(x_{t+1} | \Omega_t \backslash \{ z_j | j \leq t \} )  
 \end{equation}
...
Es importante recalcar que en este tipo de modelos dinámicos también es necesario corroborar los supuestos hechos para la estimación.
 
Dado que tenemos variables exógenas, la prueba de Ramsey es utilizada para ver si es necesario incluir más rezagos de esta variable.
 
Las pruebas de Box-Pierce y Ljung-Box pueden ser usadas para descartar correlación en los residuales, haciendo las correcciones correctas de grados de libertad. 
