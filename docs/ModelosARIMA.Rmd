...
\visible<2-> { \gamma_j & = \phi_1 \gamma_{j-1} + \phi_2 \gamma_{j-2} + \dots + \phi_p \gamma_{j-p} + E[\varepsilon_t x_{t-j}]
}
\end{align*}
\end{frame}
...
\visible<2->{Y para valores de $j>0$
...
\visible<2->{con esto podemos estimar las ecuaciones de Yule-Walker, 
...
\section{Modelos de Medias Móviles}
...
\visible<2->{Un modelo MA(1) se define como,
...
\visible<2->{reemplazando en $\rho_1$, obtenemos $| \rho_1 | \leq \frac{1}{2}$}
...
\begin{frame}{MA(q)}\label{inv:maq}
...
caigan dentro del circulo de unidad. Este problema es equivalente al que realizamos para encontrar la estacionariedad del proceso AR(p). \hyperlink{inv:ar1}{\beamergotobutton{Invertibilidad}}
...
Para escribir el proceso como un MA($\infty$) necesitamos entonces que $|\phi| < 1$, y de la misma forma para un AR($\infty$) se puede ver que $|\theta| < 1$
...
\visible<2->{O usando el operado de rezagos, $\phi(L)(1-L)^dx_t = \theta(L)\varepsilon_t $}
\end{frame}
...
\begin{frame}{Solución operador de rezagos}\label{sol:L}
...
\hyperlink{sol:lambda}{\beamergotobutton{Volver}} 


\end{frame}


\begin{frame}{Invertibilidad MA(1)}\label{inver:ma1}
...
\begin{frame}{Invertibilidad MA(1)}
ahora, si tomamos $\theta^{**} = \frac{1}{\theta^*}$ este es un proceso invertible y la auto-correlación sería

\begin{align*}
  \rho_1 & = \frac{- \theta^{**} }{ (1+\theta^{**^2})} \\
         & = \frac{- \frac{1}{\theta^*} }{1+\left(\frac{1}{\theta^*}\right)^2} \\
\visible<2->{         & = \frac{- \frac{1}{\theta^*} }{\frac{\theta^{*^2}+1}{\theta^{*^2}}} } \\
\visible<3->{       &  =  \frac{- \theta^* }{ (1+\theta^{*^2})} }
\end{align*}

\visible<3->{ por lo tanto, se puede ver que para todo proceso no-invertible corresponde un proceso invertible con el mismo coeficiente de auto-correlación}
\end{frame}


\begin{frame}{Invertibilidad AR(1)}\label{inv:ar1}
...
Si, $|\phi| <1$ podemos escribir $x_t$ como un proceso $MA(\infty)$. En general, para los modelos AR estacionariedad implica invertibilidad  \hyperlink{inv:maq}{\beamergotobutton{Volver}}  
\end{frame}
